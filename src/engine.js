...
const VANARA_DECK_DEF = [
  ...
  { id:'vinatastalon',n:"Vinata's Talon",sub:'Claw of the Mother',  t:'unit', p:4, r:'R', wave:1, txt:'ON PLAY: Deal 1 damage to an enemy Unit for every two friendly Vanaras (max 3).' },
  ...
  { id:'anjaneyaroar',n:"Anjaneya's Roar",sub:'Cry of the Son of Wind',t:'astra', p:0, r:'L', wave:1, dmgAstra:false, txt:'This round: all enemy Units −1 power; each of your Units adjacent to a friendly Unit +1.' },
];
...
function playableIndices(g, pi){
  ...
  pl.hand.forEach((c,i)=>{
    ...
    if (c.t==='astra'){
      ...
      if (c.id==='anjaneyaroar' && !opp.units.some(u=>!u.ghost) && !pl.units.some(u=>!u.ghost && adjacentUnits(pl,u).some(n=>!n.ghost))) return;   // WAVE 1 batch 15 — AoE −1 pierces protection (no astraProtected filter); illegal ONLY if BOTH effects whiff (no enemy Unit AND no friendly adjacent pair)
...
function resolveAstra(g, pi, c, targetUid){
  ...
  switch(c.id){
    ...
    case 'anjaneyaroar': {   // WAVE 1 batch 15 — dmgAstra:FALSE (debuff, not damage → NOT in ASTRA_DMG, invariant unchanged): all enemy Units −1 this round + your adjacent-pair Units +1. The −1 routes through damageUnit(cause "Anjaneya's Roar") so Holika SHARPENS it (+1 → −2, R22); Patala does NOT amplify (not in ASTRA_DMG); Ratri does NOT prevent (not dmgAstra). AoE iterates opp.units directly → pierces shield/Maya Veil (AoE convention, like Suryastra).
      ...
      // "your adjacent pairs +1" — WEAKEST reading (LOGGED, pending a ratification number): each friendly Unit with AT LEAST ONE non-ghost neighbour gets +1 ONCE (NOT +1 per neighbour). R40 non-ghost-neighbour precedent; current-power (round-scoped).
      const paired = pl.units.filter(u=>!u.ghost && adjacentUnits(pl,u).some(n=>!n.ghost));
      for (const u of paired) u.power+=1;
      if (paired.length){ emit(g,'buff',{targetUids:paired.map(u=>u.uid),amount:1,abilityName:"Anjaneya's Roar",text:'+1'}); log(g,`Anjaneya's Roar rallies ${paired.length} paired Unit(s): +1 this round.`); }
      break; }
    case 'brahmastra':
  ...
  if (c.t==='hero'){
    ...
    switch(c.id){
      ...
      case 'vinatastalon': {   // WAVE 1 batch 14 — deal N hits of 1 to ONE enemy (N = floor(non-ghost friendlies/2), capped 3; SELF-INCLUSIVE — Talon is already on the board). N separate damageUnit hits → Holika sharpens EACH (+1/hit). Cause 'Vinata's Talon' (non-astra). Does NOT respect astraProtected (a Unit's on-play damage is NOT the astra-targeting layer — R22/Maya-Veil logic).
        const n=Math.min(3, Math.floor(pl.units.filter(u=>!u.ghost).length/2));
        const foes=opp.units.filter(u=>!u.ghost);
...
function aiScoreCard(g, pi, c){
  ...
  switch(c.id){
    ...
    case 'vinatastalon': { const n=Math.min(3,Math.floor(pl.units.filter(u=>!u.ghost).length/2)); s += opp.units.some(u=>!u.ghost)? n : 0; break; }   // N chip damage
    ...
    case 'anjaneyaroar': { const foes=opp.units.filter(u=>!u.ghost).length; const paired=pl.units.filter(u=>!u.ghost && adjacentUnits(pl,u).some(n=>!n.ghost)).length; s = (foes||paired) ? foes+paired : -99; break; }   // −1 to each enemy + +1 to each paired friendly
