...
function roundEndCardEffects(g){
  ...
  for (let s=0;s<2 && !active;s++){ const pl=g.players[s];
    if ((pl.saviturUids && pl.saviturUids.length) || pl.units.some(u=>!u.ghost && RE_IDS.has(u.id)) || (pl.artifact && (pl.artifact.id==='livingbridge' || pl.artifact.id==='drownedaltar' || pl.artifact.id==='ironcrucible' || pl.artifact.id==='kalpavriksha'))) active=true; }   // + Living Bridge / Drowned Altar / Iron Crucible / Kalpavriksha (batch-16). Padmavati MOVED OUT (R54: pre-drain slot, before venomRoundEnd).
