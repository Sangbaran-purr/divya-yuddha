...
- ✅ **Audio overhaul (2nd pass)** — **synthesized MUSIC RETIRED** (owner rejected it twice). Music is now **file-only** via `window.MUSIC_URL` (looping tanpura the owner supplies), default **OFF**, with fade/mute/volume; when unset the settings show an "Add music" hint and the header speaker becomes an **SFX-only** toggle. **SFX rebuilt LOUD + layered** (whoosh / noise-thump+sine-drop / crack+debris / two-note chime / wet-hiss sweep / Mythic boom) through a **compressor limiter**, with **sidechain music-duck (−8dB)**, a **separate SFX-volume slider (default 80%)** in Settings, and a **▶ Test all SFX** button. **Showcase clipping fixed** — height-driven width (`max(150px, min(60vw, (100vh−210)·0.72, 320px))`) + portrait `750/1050` art so the full frame (head included) + name/ability fit at 375×667 / 375×812 / 1440, no clip.
- **NOTE: audio LOUDNESS is not verifiable in the headless harness (no audio output). SFX gains were raised ~2–4× with a limiter + Test-SFX button for the owner's ear; music is now the owner's own file. Judgment of "clearly audible" needs a real device.**
