...
H VRITRA             L  P8  On Play: bind an enemy Unit (0 contribution) while Vritra remains. [bind precedent] [R65 (balance round 1, ARMED — Asura still +12.7 after R63+R64): P8→P7, the second above-noise Asura lifter (ablation −2.8). Ability unchanged. ART GATE: Asuras_Hero_Vritra_P7_rLegendary] [R80 (design round 3, CONDITIONAL — ARMED, Asura +5.3 after rung one): P7→P6, honest ladder insurance (sub-noise +2.2 → −0.6). ART GATE: Asuras_Hero_Vritra_P6_rLegendary]
RATNA:
...
GUPTA:
U Setu Mason         C  P2  +1 while adjacent to another Vanara. [SUPERSEDED BY R40: '+1 while adjacent to Vanaras on both sides.'] [R68 (design round 1): +1→+2 (interior payoff; −7.2 Δbase). ART GATE: Vanaras_Unit_SetuMason_P2_rCommon] [R81 (design round 3, CONDITIONAL — ARMED, Vanara −6.5 after rung one): interior +2→+3. Δbase −6.9→−6.0. ART GATE: Vanaras_Unit_SetuMason_P2_rCommon]
...
U Gavaksha           U  P3  On Play: may swap places with a friendly Unit. [trivial] [R79 (design round 3): P3→P4 stat lift (was −9.2, swap functionally never taken — 6 fires/~460 plays; Runner precedent). Δbase −9.2→−5.9. ART GATE: Vanaras_Unit_Gavaksha_P4_rUncommon]
...
M Vault of the Sky   E  —   Move a friendly Unit anywhere; +2 this round. [trivial] [R70 (design round 1): this-round→PERMANENT +2 (R21 base+power; −6.9 Δbase, payoff was vapor). ART GATE: Vanaras_Mantra_VaultoftheSky_rEpic] [R77 (design round 3): FORMATION-NATIVE rework — the moved Unit AND its destination neighbours +2 permanent (R70 permanence alone didn't lift it, −8.0; the move was the low-value half). Composes with the R63 Bridge. ART GATE: Vanaras_Mantra_VaultoftheSky_rEpic]
...
AR The Living Bridge M  —   Round End: unbroken adjacent line of 4+ → all of them +1 permanent. [moderate — run predicate; SIM: uptime both ways] [R40: formally SIM-FLAGGED — body-count equivalence acknowledged; haircut fallback prepared] [R63 (balance round 1): RE-TEXTED to a formation-moment trigger — "When your Units FIRST form a line of 4+ this round, that run +1 permanent, once/round." LEFT the round-end hook; fires from afterAction (checkBridgeLine). Fire-rate 29.3%→52.6%; aggregate ~0 (played 6.5%, small buff)] [R78 (design round 3): grant +1→+2 (mythic-grade; epicenter −16.8 at +1×4). Constant-only. Absolute 35.9→40.6, Δbase −12.5→−10.3 (still low uptime 6%). ⚠ def txt was STALE since R63 (still read the old ROUND-END +1) — corrected. ART GATE (NEW — missed in R63): Vanaras_Artifact_TheLivingBridge_rMythic]
